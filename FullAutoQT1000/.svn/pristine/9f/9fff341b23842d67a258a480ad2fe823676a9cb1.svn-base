...
void RenfValue::InitItemControl(RenfInfo &ri)
{
    if(0 == ri.m_listItem.size()){
        ...
        for(int n=0; n<ri.m_listItem.size(); n++){
            ui->cBoxItem->addItem(ri.m_listItem.at(n));
        }
    ...
    ui(new Ui::RenfValue)
{
    ...
    valueList = m_UtilTool.ParseValue(ri.m_Value);
...
void RenfValue::on_btnOK_clicked()
{
    ...
    //参考值
    m_RenfInfo.m_Value = m_UtilTool.ProcValue(lower,upper);
    //年龄
