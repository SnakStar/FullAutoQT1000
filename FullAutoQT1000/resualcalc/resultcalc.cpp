...
void ResultCalc::calculateResult(ResultDataInfo& datainfo,unsigned int *scanData1,unsigned int *scanData2,unsigned char *pRecord,unsigned char *pIdmessgae)
{
    ...
    if( (TempMax1-Tempbase1)< PeakHighValue )   //峰值过低
    {
        //// 生成空(零)记录
...
QString ResultCalc::calculateResult2(RawTestInfo& testinfo,unsigned int *scanData1,unsigned char *pRecord,unsigned char *pIdmessgae)
{
    /*QByteArray byteScan1;
    for(int n=0; n<260; n++){
        byteScan1.append((quint8)pIdmessgae[n]);
        qDebug()<<n<<"   "<<"1:"<<(quint8)byteScan1[n];
    }*/
    //测试日志信息
    ...
    unsigned int Threshold1,Threshold2;
    unsigned int TempMaxPos1 = 0;
    unsigned int TempMaxPos2 = 0;
    ...
    testinfo.m_nCheckIntegralBreadt = TSignallength;
    CSignallength = *(pIdmessgae+35);   //质控峰积分长度
    testinfo.m_nQCIntegralBreadth = CSignallength;
    //TSignallength = 100;
    //CSignallength = 90;
    if(0 == CSignallength || CSignallength > MAX_CSignallength || *(pIdmessgae+ID_NO_CALNUM) > MAX_CALNUM)//旧的ID卡
    {
    ...
    //质控峰最小值
    testinfo.m_nQCMinHeightValue = PeakHighValue;
    for (i=0;i<4;i++)
    {
        p[i]=0;
        MaxPosition[i]=0;
    ...
    else
    {
        ...
        int nMax1Index = 0;
        if(TempMaxPos1>150){
            nMax1Index = TempMaxPos1 - TSignallength/2;
            if( nMax1Index < 0 || nMax1Index>319){
                nMax1Index = 80;
            }
            TempMax1 = scanData1[nMax1Index];
            ...
            nMax1Index = TempMaxPos1 + CSignallength/2;
            if(nMax1Index < 0 || nMax1Index > 319){
                nMax1Index = 235;
            }
            TempMax1 = scanData1[nMax1Index];
        ...
        else
        {
            ...
            strTestLogInfo = strTestLogInfo + strTempLogInfo;
            //记录波峰波谷位置，to PC Draw;
            testinfo.m_nCrestPos1 = MaxPosition[0];
            int nTroughPosLeft1 = MaxPosition[0]-CSignallength/2;
            if(nTroughPosLeft1 < 0){
                nTroughPosLeft1 = 0;
            }
            testinfo.m_nTroughPosLeft1 = nTroughPosLeft1;
            testinfo.m_nTroughPosRight1 = MaxPosition[0]+CSignallength/2;
            testinfo.m_nCrestPos2 = MaxPosition[1];
            int nTroughPosLeft2 = MaxPosition[1]-TSignallength/2;
            if(nTroughPosLeft2 < 0){
                nTroughPosLeft2 = 0;
            }
            testinfo.m_nTroughPosLeft2 = nTroughPosLeft2;
            testinfo.m_nTroughPosRight2 = MaxPosition[1]+TSignallength/2;

            //计算第一次的扫描数据的 面积积分
            area1 =0; //面积一
            qint16 nStartPosC = MaxPosition[0]-CSignallength/2;
            qint16 nEndPosC = MaxPosition[0] + CSignallength/2;
            if(nStartPosC < 0 ){
                nStartPosC = 0;
                nEndPosC = CSignallength;
            }
            for(i=nStartPosC; i< nEndPosC;i++)
            {
                if(i< m_ScanDataSize)
                {
                    area1 =area1 + scanData1[i];
            ...
            //峰左
            temp1 = scanData1[nStartPosC];
            //峰右
            temp2 = scanData1[nEndPosC];
            //面积1 - 基线下面的面积 = 峰的面积
            area1 = area1-(temp1+temp2)*CSignallength/2;
            if(area1<0){
                area1 = 0;
            ...
            //
            strTempLogInfo = QString(" [1][面积1:%1").arg(area1);
            strTestLogInfo = strTestLogInfo + strTempLogInfo;
            testinfo.m_nTest1Area1 = area1;

            area2 =0;//面积二
            qint16 nStartPosT = MaxPosition[1]-TSignallength/2;
            qint16 nEndPosT = MaxPosition[1]+TSignallength/2;
            if(nEndPosT>319){
                nStartPosT = 319-TSignallength;
                nEndPosT = 319;
            }
            for(i=nStartPosT;i<nEndPosT;i++)
            {
                if(i< m_ScanDataSize)
                {
                    area2 =area2 + scanData1[i];
            ...
            temp1 = scanData1[nStartPosT];
            temp2 = scanData1[nEndPosT];
            area2 = area2-(temp1+temp2)*TSignallength/2;
            if(area2<0){
                area2 = 0;
                testinfo.m_nTest1Area2 = 0;
            }
            testinfo.m_nTest1Area2 = area2;
            strTempLogInfo = QString(",面积2:%1]").arg(area2);
            strTestLogInfo = strTestLogInfo + strTempLogInfo;
            if(0 == area1 || 0 == area2){
                value1 = 0;
            }else{
                //int nCalcNum = pIdmessgae[23];
                int nCalcNum = testinfo.m_nComputeMothed;
                if(nCalcNum == 0){
                    value1 = (area2*bit5)/area1; //比值1
                }else{
...
{
    int nLen = byteIDCardData.size()-3;
