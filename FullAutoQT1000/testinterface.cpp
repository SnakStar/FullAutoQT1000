    ...
    ui(new Ui::TestInterface)
{
    ...
    m_CheckPanelsSerial->setObjectName("m_CheckPanelsSerial");
    m_ControlPanelsSerial->setObjectName("m_ControlPanelsSerial");
    //初始化扫码模式
    InitScanMode();
    //连接设置界面扫码槽
    ...
    //串口数据重发机制
    connect(&m_TimerReSend,SIGNAL(timeout()), this, SLOT(ReSendSerialData()) );
    //设置日志字体大小
...
void TestInterface::StartUnstallCard(const QByteArray &AckData)
{
    ...
    //开启重发-控制板
    m_TimerReSend.start(RESENDTIME);
    m_nReSendSerial = 1;
    m_nReSendCount = 0;
}
    ...
    for(int n=0 ;n<nCountFrame; n++){
        ...
        if(FillValidSerialData(byarrSingleData,CurrData)){
            ...
            case PL_FUN_AM_TEST_RESULT :
                //响应ACK
                ResponseACK(m_CheckPanelsSerial,PL_FUN_AM_TEST_RESULT);
                //收到测试结果
            ...
            case PL_FUN_AM_START_UNSTALL_COMPLETE:{
                //响应ACK
                ResponseACK(m_CheckPanelsSerial,PL_FUN_AM_START_UNSTALL_COMPLETE);
                //卸卡完成
            ...
            case PL_FUN_AM_ID_DEVICE_SCANOFFSET_COMPLETE:
                //响应ACK
                ResponseACK(m_CheckPanelsSerial,PL_FUN_AM_ID_DEVICE_SCANOFFSET_COMPLETE);
                //激光模块偏移量
                emit sendDebugHardware2ScanOffsetData(CurrData.m_Data);
                break;
            case PL_FUN_AM_ID_DEBUG_EXIMOTOR_COMPLETE:{
                //响应ACK
                ResponseACK(m_CheckPanelsSerial,PL_FUN_AM_ID_DEBUG_EXIMOTOR_COMPLETE);
                //退卡电机信息
            ...
            case PL_FUN_AM_ID_DEVICE_CHECKHARDVERSION:{
                //响应ACK
                ResponseACK(m_CheckPanelsSerial,PL_FUN_AM_ID_DEVICE_CHECKHARDVERSION);
                //检测板硬件版本号
            ...
            case PL_FUN_AM_STATUS_CLEANOUT:{
                //响应ACK
                ResponseACK(m_CheckPanelsSerial,PL_FUN_AM_STATUS_CLEANOUT);
                //清洗液状态
            ...
            case PL_FUN_AM_STATUS_EFLUENT:{
                //响应ACK
                ResponseACK(m_CheckPanelsSerial,PL_FUN_AM_STATUS_EFLUENT);
                //废液状态
...
void TestInterface::RecvControlPanelsData()
{
    ...
    for(int n=0 ;n<nCountFrame; n++){
        ...
        if(FillValidSerialData(byarrSingleData,CurrData)){
            ...
            case PL_FUN_ALARM:
                //响应ACK
                ResponseACK(m_ControlPanelsSerial,PL_FUN_ALARM);
                //仪器报警
            ...
            case PL_FUN_AM_SCAN_RESULT:
                //响应ACK
                ResponseACK(m_ControlPanelsSerial,PL_FUN_AM_SCAN_RESULT);
                //收到扫码结果
            ...
            case PL_FUN_AM_SAMPLE_COMPLETE:
                //响应ACK
                ResponseACK(m_ControlPanelsSerial,PL_FUN_AM_SAMPLE_COMPLETE);
                //收到加样完成，进行倒计时
            ...
            {
                //响应ACK
                ResponseACK(m_ControlPanelsSerial,PL_FUN_AM_PREPARE_TEST_COMPLETE);
                //收到准备测试完成，发送开始测试
            ...
            {
                //响应ACK
                ResponseACK(m_ControlPanelsSerial,PL_FUN_AM_PREPARE_UNSTALL_COMPLETE);
                //卸卡准备完成，开始卸卡
            ...
            case PL_FUN_AM_ID_CARD_DATA:
                //响应ACK
                ResponseACK(m_ControlPanelsSerial,PL_FUN_AM_ID_CARD_DATA);
                //收到ID卡数据
                ParseIDCardData(CurrData.m_Data);
                break;
            case PL_FUN_AM_ID_EMER_SCANRESULT:
                //响应ACK
                ResponseACK(m_ControlPanelsSerial,PL_FUN_AM_ID_EMER_SCANRESULT);
                //急诊扫码
            ...
            case PL_FUN_AM_ID_DEBUG_PUSHMOTORRECV:
                //响应ACK
                ResponseACK(m_ControlPanelsSerial,PL_FUN_AM_ID_DEBUG_PUSHMOTORRECV);
                //推卡电机参数结果
                emit sendDebugHardwarePushMotorData(CurrData.m_Data);
                break;
            case PL_FUN_AM_ID_DEBUG_DIALPARAMRECV:
                //响应ACK
                ResponseACK(m_ControlPanelsSerial,PL_FUN_AM_ID_DEBUG_DIALPARAMRECV);
                //转盘参数结果
                emit sendDebugHardwareDialData(CurrData.m_Data);
                break;
            case PL_FUN_AM_ID_DEBUG_LOADINGARMRCEV:
                //响应ACK
                ResponseACK(m_ControlPanelsSerial,PL_FUN_AM_ID_DEBUG_LOADINGARMRCEV);
                //加样臂参数结果
            ...
            {
                //响应ACK
                ResponseACK(m_ControlPanelsSerial,PL_FUN_AM_ID_DEVICE_HARDSHUTDOWN);
                //仪器硬件关机
                QString strContent;
            ...
            case PL_FUN_AM_ID_DEVICE_HARDVERSION:
                //响应ACK
                ResponseACK(m_ControlPanelsSerial,PL_FUN_AM_ID_DEVICE_HARDVERSION);
                //硬件版本号
                emit sendHardwareVersionData(CurrData.m_Data);
                break;
            case PL_FUN_AM_ID_DEBUG_HARDWARE_BUFFERSIZE_COMPLETE:
                //响应ACK
                ResponseACK(m_ControlPanelsSerial,PL_FUN_AM_ID_DEBUG_HARDWARE_BUFFERSIZE_COMPLETE);
                //缓冲液大小参数设置完成
            ...
            {
                //响应ACK
                ResponseACK(m_ControlPanelsSerial,PL_FUN_AM_ID_REQUEST_REAGENTPOS);
                QElapsedTimer t;
                t.start();
                while(t.elapsed()<50);
                //试剂位置请求
...
/********************************************************
 *@Name:        ParseRequestDialPostion
 *@Author:      HuaT
 *@Description: 分配样本位置给下位机
 *@Param1:      收到的请求数据
 *@Return:      无
 *@Version:     1.0
 *@Date:        2018-5-21
...
/********************************************************
 *@Name:        ReSendSerialData
 *@Author:      HuaT
 *@Description: 无响应ACK时，重发串口命令数据
 *@Param1:      无
 *@Return:      无
 *@Version:     1.0
 *@Date:        2020-02-17
********************************************************/
void TestInterface::ReSendSerialData()
{
    if(m_nReSendCount >= RESENDCOUNT){
        m_nReSendCount = 0;
        m_TimerReSend.stop();
        return;
    }
    if(m_nReSendSerial == 1){
        m_ControlPanelsSerial->write(m_byarrReSendData);
        qDebug()<<"Control-ReSend-Data:"<<m_byarrReSendData.toHex().toUpper();
    }else if(m_nReSendSerial == 2){
        m_CheckPanelsSerial->write(m_byarrReSendData);
        qDebug()<<"Check-ReSend-Data:"<<m_byarrReSendData.toHex().toUpper();
    }
    m_nReSendCount++;
}
 ...
 *@Date:        2020-02-19
********************************************************/
void TestInterface::ResponseACK(QextSerialPort* ser,qint8 nFuncIndex)
{
    QByteArray byarr;
    byarr.append(IntToBytes(nFuncIndex));
    byarr.append(IntToBytes(0));
    byarr.append(IntToBytes(0));
    byarr.append(IntToBytes(0));
    SendSerialData(ser,PL_FUN_ACK,byarr);
}
...
void TestInterface::SendSerialData(QextSerialPort *ser, quint16 nFuncIndex,QByteArray arrData)
{
    ...
    ser->write(byarrMsg);
    m_byarrReSendData.clear();
    m_byarrReSendData = byarrMsg;
    //debug output
    if(ser->objectName() == "m_CheckPanelsSerial"){
        m_nReSendSerial = 1;
        DebugOutput(msgd.m_FuncIndex,msgd.m_Data,"Check-Send");
    }else{
        m_nReSendSerial = 2;
        DebugOutput(msgd.m_FuncIndex,msgd.m_Data,"Control-Send");
    }
...
void TestInterface::DebugOutput(quint16 nFuncIndex, QByteArray byarrContentData, QString type){
    QString strResult;
    if(type == "Check-Send"){
        strResult = "Check-Send-Data:";
    }else if(type == "Control-Send"){
        strResult = "Control-Send-Data:";
...
void TestInterface::ParseControlPanelsAck(const QByteArray& AckData)
{
    ...
    case PL_FUN_AM_PREPARE_UNSTALL:
        //准备卸卡
    case PL_FUN_AM_ID_DEBUG_PUSHMOTOR:
        //推卡电机参数设置
    case PL_FUN_AM_ID_DEBUG_DIALPARAM:
        //转盘参数设置
    case PL_FUN_AM_ID_DEBUG_LOADINGARMSET:
        //加样臂参数设置
    case PL_FUN_AM_ID_DEBUG_DIAPHRAGM:
        //隔膜泵参数设置
    case PL_FUN_AM_ID_DEBUG_SOLENOID:
        //电磁阀参数设置
    case PL_FUN_AM_ID_DEBUG_PLUNGERPUMP:
        //柱塞泵参数设置
    case PL_FUN_AM_ID_DEVICE_SOFTSHUTDOWN:
        //仪器软件关机
    case PL_FUN_AM_ID_REQUEST_REAGENTPOS_COMPLETE:
        //试剂位置请求完成
        m_TimerReSend.stop();
        m_byarrReSendData.clear();
        break;
